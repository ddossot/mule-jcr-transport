...
public class JcrMessageRequester extends AbstractMessageRequester {
     ...
     * using the path of the endpoint. Alternatively, an event property (
     * <code>JcrConnector.JCR_NODE_UUID_PROPERTY</code>) can be used to specify
     * the UUID to use to select the target node: if this is done, the endpoint
     * URI will be ignored. A third option is to define a query (with
     * queryStatement and queryLanguage) that will be used to select the node.
     * Then, if any optional relative paths have been specified as event
     * properties for the current <code>MuleEvent</code> (
     * <code>JcrConnector.JCR_NODE_RELPATH_PROPERTY</code> and/or
     * <code>JcrConnector.JCR_PROPERTY_REL_PATH_PROPERTY</code>), they will be
     * used to navigate to the actual item to use.
     * </p>
     * 
     * <p>
     * The second step consists in applying any <code>JcrNodeNameFilter</code>
     * or <code>JcrPropertyNameFilter</code> that could have been defined on the
     ...
     * <ul>
     * <li>For a single-valued property, the payload will be the property value.
     * </li>
     * <li>For a multi-valued property, the payload will be a <code>List</code>
     * of values.</li>
     * <li>For a node, the payload will be a <code>Map</code> of property names
     * and property values (for these values, the previous two rules will
     * apply).</li>
     * </ul>
     * </p>
     * 
