...
public class JcrMessageDispatcher extends AbstractMessageDispatcher {
	...
	private String nodeNamePatternFilter;

	private String propertyNamePatternFilter;
	...
	public UMOMessage doSend(final UMOEvent event) throws Exception {
		...
		Item targetItem = alwaysCreate ? null : getTargetItem(session, event,
				true);
	...
	private String getNodeTypeName(final UMOEvent event) {
		...
		Object nodeTypeNameProperty = event.getProperty(
	...
	private Item getTargetItem(final Session session, final UMOEvent event,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Getting target item with nodeUUID='" + nodeUUID
		...
		if (nodeUUID != null) {
			...
			targetItem = getTargetItemFromPath(session, nodeRelPath,
					propertyRelPath);
		}
	...
	private Item getTargetItemFromQuery(final Session session,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Query : " + context + " returned: "
	...
	private Item getTargetItemFromNodeIterator(final String pathContext,
		...
		if (nodes.getSize() == 0) {
			...
			if (nodes.getSize() > 1) {
				logger.warn(JcrMessages.moreThanOneNodeFor(pathContext)
						.getMessage());
			}
	...
	private Item getTargetItemFromPath(final Session session,
		...
		if ((logger.isDebugEnabled()) && (targetItem.getItem() == null)) {
			logger.debug(JcrMessages.noNodeFor(targetItem.getAbsolutePath())
					.getMessage());
		}
